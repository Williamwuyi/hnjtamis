...
ClassDefine('modules.hnjtamis.exampaper.Exampaper', {
	...
	initComponent : function() {
		...
		me.updateBt = Ext.create("Ext.Button", {
							...
							handler : function() {
								...
								if(state==null || state=='' || state === '5' || state === '20' ){//|| state === '15'
									...
									Ext.Msg.alert('提示', '考试安排已发布，不能进行修改！');
								}
		...
		this.otherOperaters.push(Ext.create("Ext.Button", {
							...
							handler : function() {
								...
								if(state==null || state=='' || state === '5' || state === '20'){// || state === '15'
									...
									Ext.Msg.alert('提示', '上报或发布状态，不能进行删除！');
								}
		...
		if(me.cfg == 'base'){
			...
			this.otherOperaters.push({  
				...
				handler : function() {//按钮事件
						...
						//状态 5:保存10:上报15:发布20:打回
						if(stCreateNow == 'T'){
							Ext.Msg.alert('提示', '该考试安排正在初始化，不能进行修改！');
		...
		this.openFormWin = function(id,callback,readOnly,data,term,oper){
			...
			formConfig.formUrl = "exam/exampaper/saveForExampaperFormAction!save.action";//保存
			...
			formConfig.items.push({
				...
				name:'publicId',
				fieldLabel:"信息发布名称",
				selectUrl:'exam/exampaper/queryExamPublicListForExampaperListAction!queryExamPublicList.action',
				valueField:'publicId',
	...
	yz : function(){
		var me = this;
		var examFormStore = me.examForm.getStore();
		if(examFormStore.getCount()<1){
			Ext.Msg.alert('信息', '必须包含一个科目信息！');
			return false;
		}
		for(var i=0;i<examFormStore.getCount();i++){
			var record = examFormStore.getAt(i);
			var examNameValue = record.get("examName");
			var examPaperTypeValue = record.get("examPaperType");
			var examStartTimeValue = record.get("examStartTime");
			var examEndTimeValue = record.get("examEndTime");
			var beforeTimeValue = record.get("beforeTime");
			var banTimeValue = record.get("banTime");
			var passScoreValue = record.get("passScore");
			var testpaperIdValue = record.get("testpaper")["testpaperId"];
			if(examNameValue == null || String(examNameValue).trim()==""){
				Ext.Msg.alert('信息', '科目不能为空！');
				return false;
			}
			if(examPaperTypeValue==null || String(examPaperTypeValue).trim()==""){
				Ext.Msg.alert('信息', '出题方式不能为空！');
				return false;
			}
			...
			if( examEndTimeValue == undefined || examEndTimeValue == null){
				...
				return false;
			}else if(examStartTimeValue>=examEndTimeValue){
				Ext.Msg.alert('信息', '考试开始时间必须小于考试结束时间！');
				return false;
			}
			if(beforeTimeValue==null || String(beforeTimeValue).trim()==""){
				Ext.Msg.alert('信息', '提前进入时间不能为空！');
				return false;
			}else if(Number(beforeTimeValue)<0){
				Ext.Msg.alert('信息', '提前进入时间不能负数！');
				return false;
			}
			if(banTimeValue==null || String(banTimeValue).trim()==""){
				Ext.Msg.alert('信息', '考试后禁止进入时间不能为空！');
				return false;
			}else if(Number(banTimeValue)<0){
				Ext.Msg.alert('信息', '考试后禁止进入时间不能负数！');
				return false;
			}
			if(passScoreValue==null ||  String(passScoreValue).trim()==""){
				Ext.Msg.alert('信息', '及格分数线不能为空！');
				return false;
			}else if(Number(passScoreValue)<0){
				Ext.Msg.alert('信息', '及格分数线不能负数！');
				return false;
			}
			if(testpaperIdValue==undefined || testpaperIdValue ==null){
				Ext.Msg.alert('信息', '试卷选择不能为空！');
				return false;
			}
	...
	setExamForm : function(readOnly){
		var me = this;
		me.examForm = ClassCreate('base.core.EditList',{
				colspan : 2,
				fieldLabel : '考试科目',
				name : 'examFormList',
				xtype : 'editlist',
				...
				columns : [
							{
								...
								editor : {
									...
									selectTypeName:'试卷',
									selectUrl : "exam/testpaper/testPaperTreeListForTestpaperListAction!testPaperTreeList.action",
									readOnly : readOnly
								},
								renderer : function(value){
									//console.log(value);
									return value['testpaperName'];
								}
								//,listeners:{collapse:function(){
								//	me.examForm.getView().refresh();
								//}}
							}
				           ]
			});
			
	    me.examForm.addRecord = function(){
		    var thisme = me.examForm;
			var rowIndex = thisme.store.getCount();
			if (rowIndex >= thisme.allowMaxSize && thisme.allowMaxSize > 0) {
				Ext.Msg.alert('提示','超过最大记录数'+ thisme.allowMaxSize+ '！');
				return;
			}
			var row = {};
			var _examArrangeName = me.form.getForm().findField("examArrangeName").getValue();
			for (var i = 0; i < thisme.columns.length; i++) {
				if(thisme.columns[i].name == 'examName'){
					if(rowIndex == 0){
					  row[thisme.columns[i].name] = _examArrangeName;
					}else{
						row[thisme.columns[i].name] = _examArrangeName+"_"+rowIndex;
					}
				    
				}else if(thisme.columns[i].name == 'examPaperType'){
					row[thisme.columns[i].name] = '10';
				}else{
					row[thisme.columns[i].name] = thisme.columns[i].defaultValue;
				}
			}
			var insertIndex = thisme.store.indexOf(thisme.getSelectionModel().getLastSelected())+1;
			if(insertIndex==0)
			   insertIndex = rowIndex;
			thisme.store.insert(insertIndex, row);
			thisme.getSelectionModel().select(insertIndex);
	    }
		return me.examForm;
	},
	setExamPropertyText : function(oper){
		...
		if(_publicId!=undefined && _publicId!=null && _publicId!=""){
			Ext.Ajax.request({
				url : 'base/exampublic/findForExamPublicFormAction!find.action',
				method : 'post',
				timeout: 60000,
				params : {
				   id : _publicId
	...
	setExamUserTable : function (id,parentMe){
	    ...
	    me.examUserTableGrid = new Ext.grid.GridPanel({
	    	...
	    	columns:[
	    		...
	    		{header:"准考证",dataIndex:"inticket",sortable:false, menuDisabled:true,width:75,align:"center",
	    		renderer:function(value, metadata, record){metadata.style="white-space: normal !important;"; return value;}},
	    		{header:"身份证",dataIndex:"idNumber",sortable:false, menuDisabled:true,width:65,align:"center",
	    		renderer:function(value, metadata, record){metadata.style="white-space: normal !important;"; return value;}},
	    		{header:"密码",dataIndex:"examPassword",sortable:false, menuDisabled:true,width:34,align:"center",
	    		renderer:function(value, metadata, record){metadata.style="white-space: normal !important;"; return value;}},
	...
	showExamReviewer:function(id,name){
		...
		me.editorYJR = Ext.widget('selecttree',{
					...
					selectUrl : "base/exammarkpeopleinfo/queryExamReviewerTreeForExamMarkPeopleInfoListAction!queryExamReviewerTree.action?examId="+id,//?organId="+base.Login.userSession.currentOrganId,
					selectEventFun:function(combo){
		...
		me.editorYJR.store.on("load",function(){
			if(me.editorExammarkpeople){
			    ...
			    me.editTable = ClassCreate('base.core.EditList',{
					...
					deleteOperater : true,
					enableMoveButton:false,
					viewConfig:{height:188,autoScroll:true},
					columns : [{
								width : 0,
