	...
	//进行倒计时，并按时提交
	 		...
	 		for(var i=0;i<me.pageRowMax;i++){
	 			...
	 			for(var k=0;k<me.pageColumnMax;k++){
	 				...
	 				if(sortHtml>me.userThemes.length){
	 					me.pageSortHtmlStr += '<td width="39">&nbsp;</td>';
	 				}else if(me.currIndex == (sortHtml-1)){//当前题目
	 					me.pageSortHtmlStr += '<td width="39" id="themeBt_'+(sortHtml-1)+'" class="R_list_dangq" onclick="theme.setThemeHtml('+(sortHtml-1)+')">'+sortHtml+'</td>';
	 				}else if(me.userThemes[sortHtml-1].signFlag == true){//标记
	 					me.pageSortHtmlStr += '<td width="39" id="themeBt_'+(sortHtml-1)+'" class="R_list_biaoj" onclick="theme.setThemeHtml('+(sortHtml-1)+')">'+sortHtml+'</td>';
	 				}else if(me.userThemes[sortHtml-1].userAnswer && me.userThemes[sortHtml-1].userAnswer!=''){//已答
	 					me.pageSortHtmlStr += '<td width="39" id="themeBt_'+(sortHtml-1)+'" class="R_list_yida" onclick="theme.setThemeHtml('+(sortHtml-1)+')">'+sortHtml+'</td>';
	 				}else{
	 					me.pageSortHtmlStr += '<td width="39" id="themeBt_'+(sortHtml-1)+'" onclick="theme.setThemeHtml('+(sortHtml-1)+')">'+sortHtml+'</td>';
	 				}
 	...
 	flushTabAndHtml : function(){
 		...
 		if(me.pageIndex !=nowIndex){
 			//console.log(document.getElementById("themeTab"+me.pageIndex))
 	...
 	setjd : function (){
  	...
  	themeOnKey : function(){
  				var tihtml = document.getElementById("sjth").innerHTML;
				if(tihtml == '等待时间'){
					return;
				}
				var keCodeTemp = event.keyCode;    
				if(onlineObject.getTheme().Type==5){
					switch(keCodeTemp){
   					case 37:
   						up();
   				...
   				switch(keCodeTemp){
   					...
   					case 97:
   						//alerttmp = "1键";
   						setKeyBoardAwe(onlineObject.getTheme().Type,0);
   						break;
   					case 50:
   					case 98:
   						//alerttmp = "2键";
   						setKeyBoardAwe(onlineObject.getTheme().Type,1);
   						break;
   					case 51:
   					case 99:
   						//alerttmp = "3键";
   						setKeyBoardAwe(onlineObject.getTheme().Type,2);
   						break;
   					case 52:
   					case 100:
   						//alerttmp = "4键";
   						setKeyBoardAwe(onlineObject.getTheme().Type,3);
   						break;
   					case 65:
   						//alerttmp = "a键";
   						setKeyBoardAwe(onlineObject.getTheme().Type,0);
   					...
   					case 101:
   						//alerttmp = "e键";
   						setKeyBoardAwe(onlineObject.getTheme().Type,4);
   						break;
   					case 38:
   						//前20题
   						for(var i=0;i<20;i++){
