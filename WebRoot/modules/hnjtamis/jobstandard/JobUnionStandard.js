 ...
 * 岗位标准管理 
 * Date : 2015.3.27
 * Desc : 岗位标准模块-岗位达标信息维护
 * Author: Wangyong
 */
ClassDefine('modules.hnjtamis.jobstandard.JobUnionStandard', {
	...
	initComponent : function() {
		...
		this.columns = [{
					...
					name : 'standardTerms.standardname',
					header : '子模块',
					width : 2
				}, {
					name : 'isavailable',
		...
		this.keyColumnName = "jusdid";// 主健属性名
		this.viewOperater = true;
		this.addOperater = true;
		this.deleteOperater = true;
		this.updateOperater = true;
		this.listUrl = "jobstandard/jobunionstandard/listForJobUnionStandardListAction!list.action";// 列表请求地址
		this.deleteUrl = "jobstandard/jobunionstandard/listForJobUnionStandardListAction!delete.action";// 删除请求地址
		/*
		...
		this.otherOperaters.push({
							...
							handler : function() {
								...
								var config = {
									  ...
									  height:500,
									  //数据提取地址
					    		      selectUrl:"jobstandard/terms/treeForStandardTermsListAction!tree.action?jobidTerm="+id,
					    		      checked:true,//是否复选
					    		      selectType:'standardterm',//指定可选择的节点
					    		      selectTypeName:'条款',
		...
		*/
		...
		this.openFormWin = function(id, callback, readOnly,data,term,oper) {
			...
			formConfig.formUrl = "jobstandard/jobunionstandard/saveForJobUnionStandardFormAction!save.action";
			formConfig.findUrl = "jobstandard/jobunionstandard/findForJobUnionStandardFormAction!find.action";
			...
			formConfig.items.push({
						...
						readOnly : readOnly,
						nameKey : 'standardid',
						nameLable : 'standardname',
						//readerRoot : 'children',
						keyColumnName : 'id',
						titleColumnName : 'title',
						childColumnName : 'children',
						//selectType : 'standardterm',
						///selectTypeName : '标准条款',
						selectUrl : 'jobstandard/terms/treeForStandardTermsListAction!tree.action'
					});
			*/
		    formConfig.items.push({
						colspan : 1,
			...
			var editTable = Ext.widget('editlist',{
				colspan : 2,
				fieldLabel : '选择子模块',
				keyColumnName : 'standardid',
				name : 'jobstandardterms',
				xtype : 'editlist',
				addOperater : false,
				...
				otherOperaters:[{
					...
					handler : function() {
						...
						var config = {
							  ...
							  height:500,
							  //数据提取地址
			    		      selectUrl:"jobstandard/terms/treeForStandardTermsListAction!tree.action?jobidTerm="+data.jobscode,
			    		      checked:true,//是否复选
			    		      selectType:'standardterm',//只有角色结点数据才算
			    		      selectTypeName:'标准子模块',
			    		      levelAffect:'20',//上下级复选框的影响策略
						      keyColumnName : 'id',
							  titleColumnName : 'title',
							  childColumnName : 'children',
						...
						base.model.Tree.openWin(config,function(ids,selectObject){//ID数组，对象数组
						    ...
						    if (ids != null && ids != "") {
							    ...
							    for(var i = 0; i < idarray.length; i++){
									...
									////表格赋值
									var row = {};
									row['standardid']=idarray[i];
									row['standardname'] = selectObject[i].title;
									///row['contents'] = "f";
									////row['creationDate'] = "";
