...
ClassDefine('modules.hnjtamis.base.themebank.ThemeBank', {
	...
	initComponent : function() {
		...
		this.readerRoot = 'themeBanks';
		this.otherOperaters = [];//其它扩展按钮操作
		this.listUrl = "base/themebank/listForThemeBankListAction!list.action";// 列表请求地址
		this.deleteUrl = "base/themebank/deleteForThemeBankListAction!delete.action";// 删除请求地址
		...
		this.openSortWin = function(record,term,store,callback){
			...
			sortConfig.sortlistUrl = "base/themebank/subListForThemeBankListAction!subList.action?parentId="+(record?record.themeBankId:'');
			sortConfig.jsonParemeterName = 'themeBanks';
			sortConfig.saveSortUrl = 'base/themebank/saveSortForThemeBankFormAction!saveSort.action';
		...
		this.openFormWin = function(id, callback, readOnly,data,term,oper) {
			...
			formConfig.readOnly = readOnly;
			formConfig.fileUpload = true;
			formConfig.formUrl = "base/themebank/saveForThemeBankFormAction!save.action";
			formConfig.findUrl = "base/themebank/findForThemeBankFormAction!find.action";
			...
			var parentThemeBank = Ext.widget('selecttree',{
						...
						readOnly : readOnly,
						selectUrl : "base/themebank/treeForThemeBankListAction!tree.action?notHaveIds="+(oper=='update'&&data?data.themeBankId:''),
						width : 560,
						labelWidth:110
					});
