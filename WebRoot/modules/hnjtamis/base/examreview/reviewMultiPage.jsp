...
<body style="background-color:#FFFFFF;" id="mybody">
	<h1>${exam.examName } 考生答卷</h1>
	<c:set var="isHaveTheme" value="F"></c:set>
	<c:forEach items="${resultMap }" var="tixing" varStatus="tixingNum">
		...
		<c:forEach items="${tixing.value }" var="timu" varStatus="timuNum">
			...
			<h3 class="examineeAnswerClass">
				考生答案:<c:set var="isHaveTheme" value="T"></c:set>
			...
			<div class="commentDiv">
				请评分:<input type="text" name="${timu.examTestpaperThemeId }score" onblur="checkNum(this,this.value)" maxlength="6" style="width:70px;"/>
				评语: <input type="text" name="${timu.examTestpaperThemeId }comment" oninput="OnInput (event)" onpropertychange="OnPropChanged (event)" maxlength="100" style="width:500px;"/>
			</div>
		</div>
		</c:forEach>	
	</c:forEach>
	<div class="bottomDiv">
		<c:if test="${isHaveTheme eq 'T'}">
		<button id="reviewCommitBtn">阅卷结束</button>
		</c:if>
		<c:if test="${isHaveTheme ne 'T'}">
			<font style="font-size: 15px;color: red;">没有找到需要阅卷的试题！</font>
		</c:if>
		<br><br>
	</div>
	<script type="text/javascript">
		...
		Ext.onReady(function(){
			...
			Ext.get('reviewCommitBtn').on('click',function(){
				...
				//验证通过提交后台  
				commitData(dataArray,remoteTimuIds);
			});
			</c:if>
		});
		//提交
		function commitData(dataArray,remoteTimuIds){
			var remoteData = "";
			...
			Ext.Ajax.request({
				url:'base/examreview/savaSingleReviewForExamReviewFormAction!savaSingleReview.action',
				method:'POST',
				params:{
