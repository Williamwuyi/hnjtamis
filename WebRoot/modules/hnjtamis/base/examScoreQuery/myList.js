 ...
 * 标准模板的列表页面类
 */
Ext.define('modules.hnjtamis.base.examScoreQuery.myList', {
	...
	initComponent : function() {
		...
		me.store.on("load", function(mystore,records,success) {
			...
			for(var i=4;i<me.columns.length;i++){
				var isHide = true;
				if(!!record && record['data']['typeList']!=null && record['data']['typeList'].length>0){
					for(var j=0;j<record['data']['typeList'].length;j++){
						if(me.columns[i]['text'].indexOf(record['data']['typeList'][j])!=-1){
							isHide = false;
							break;
						}
					}
