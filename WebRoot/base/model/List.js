...
Ext.define('base.model.List', {
	...
	initComponent : function() {
		var me = this;
		if(me.flowCode){
			Ext.syncRequire('modules.workflow.Test');
			if(!me.flowStateField||!me.flowStateName){
				Ext.Msg.alert('提示', '请定义流程状态字段名flowStateField和流程状态名称flowStateName！');
				return;
			}
			var stateSelect = modules.workflow.Test.getStateSelect(me.flowStateName,me.flowCode,me.flowStateField);
			me.columns.push({
					name : me.flowStateField,
					header : me.flowStateName,
					width : 2,
					renderer:stateSelect.renderer
				});
		   me.terms.push(stateSelect);
		   if(!me.otherOperaters) 
		     me.otherOperaters=[];
		   var flowbuttons = modules.workflow.Test.getFlowTaskButtons(me,me.flowCode);
		   for(var x=0;x<flowbuttons.length;x++)
		     me.otherOperaters.push(flowbuttons[x]);//增加审核流程按钮
		}
		if(me.enableCheck)
		...
		me.store.on("load", function(mystore,records,success) {
			...
			if(me.flowCode)
			  modules.workflow.Test.showHideButtons(me.flowCode,me.otherOperaters);
		});
				...
				if(f.columns){
					...
					if (i == 0&&level==0&&!rowNumber){
						if(!me.enableRowNumber){
							...
							f.text="序号";
							f.width = grabl_numberColumnWidth||50;
							f.titleAlign="center";
							rowNumber=true;
						}
					}
		...
		if (termFormItmes.length > 0) {
			...
			me.termFormWin = new Ext.Window({
				    ...
				    var oneTerm=Ext.applyIf(me.terms[0], {
							                labelAlign : 'right',
									        labelWidth : 70,
											id : me.defaultTermId
								});
					me.tbar.push(oneTerm);
					me.defaultTermName = oneTerm.name;
					me.defaultTermId = oneTerm.id;
					me.tbar.push("-");
		...
		me.tbar.push('->');
		if(me.otherOperaters==undefined || me.otherOperaters ==null){
			me.otherOperaters = [];
		}
	    if(me.viewOperater)
	...
	termQueryFun : function(setvalued,type) {
		var me = this;
		if (me.termForm.getForm().isValid()&&
		   (!me.validate||me.validate(me.termForm.getForm().getValues(false)))){
			if(setvalued==undefined) setvalued=true;
			if(type==undefined) type='first';
			if(setvalued){
				if(me.termForm.items.length>0){
				   var defaultTerm = Ext.getCmp(me.defaultTermId);
				   var value = defaultTerm.getValue();
				   if(defaultTerm.format){
				   	   value = Ext.util.Format.date(value, defaultTerm.format);
				   }
				   me.termForm.getForm().findField(me.defaultTermName).setValue(value);
				   me.termFormWin.hide();
				}
	...
	extGridToExcel : function(){    
         try{   
            ...
            if(Ext){    
                Ext.Msg.show({    
                    title:'提示',msg:'请设置IE的菜单\'工具\'->Internet选项->安全->自定义级别->\'对未标记为可安全执行脚本ActiveX控件初始化并执行脚本\'->选择[启用]&nbsp;&nbsp;就可以生成Excel',    
                    buttons:Ext.Msg.OK,    
                   icon:Ext.Msg.INFO    
                });    
            }else{    
	...
	exportXls : function(){
        ...
        if (isIE || Ext.isSafari
            ...
            //window.open("openXls.jsp?fileName=导出EXCEL&xls="+this.headerCt.el.dom+this.body.dom);
	...
	mergeCells : function(records) {
		...
		var merge = function(rowspanObj, removeObjs) { // 定义合并函数
			if (rowspanObj.cellspan != 1) {
				...
				records[rowspanObj.tr].data[grid.columns[rowspanObj.td-(grid.enableCheck?1:0)].dataIndex+"_row"]=rowspanObj.rowspan;
				records[rowspanObj.tr].data[grid.columns[rowspanObj.td-(grid.enableCheck?1:0)].dataIndex+"_cell"]=rowspanObj.cellspan;
				//records[rowspanObj.tr].data[grid.columns[rowspanObj.td].dataIndex+"_index"]=1;
				td.vAlign = "middle";
				td.firstChild.style.marginTop=((rowspanObj.rowspan*21/2-10)+'px');
				td.firstChild.innerHTML = rowspanObj.html;
				records[rowspanObj.tr].data[grid.columns[rowspanObj.td-(grid.enableCheck?1:0)].dataIndex+"_text"]=rowspanObj.html;
				Ext.each(removeObjs, function(obj) { // 隐身被合并的单元格
					arrayTd = arrayTr[obj.tr].getElementsByTagName("td");
					arrayTd[obj.td].style.display = 'none';
					records[obj.tr].data[grid.columns[obj.td-(grid.enableCheck?1:0)].dataIndex+"_row"]=0;					
				});
			}
		};
			...
			if(i>0){
				...
				for(var col=(grid.enableCheck?1:0);col<tdCount;col++) { // 逐列操作
					...
					if(!mergeData)
					    continue;
					var pre = i-1;
					while(pre>=0&&!records[pre].mergeData[field]){
						pre--;
					}
			        if(pre>=0){
			        	...
			        	if(preMergeData.cellspan==mergeData.cellspan){
							...
							if (merged) {
								...
								records[preMergeData.tr].data[grid.columns[preMergeData.td-(grid.enableCheck?1:0)].dataIndex+"_row"]=td.rowSpan;
								
								td.vAlign = "middle";
				                td.firstChild.style.marginTop=((td.rowSpan*21/2-10)+'px');
				                arrayTr[i].getElementsByTagName("td")[col].style.display = 'none';
				                
				                records[i].data[grid.columns[col-(grid.enableCheck?1:0)].dataIndex+"_row"]=0;
				                
				                td.firstChild.innerHTML=preMergeData.html;
				                records[preMergeData.tr].data[grid.columns[preMergeData.td-(grid.enableCheck?1:0)].dataIndex+"_text"]=preMergeData.html;
							}
			        	}
			        }
				}
