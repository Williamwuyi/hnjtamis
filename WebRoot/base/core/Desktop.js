...
Ext.define('Ext.ux.desktop.Desktop', {
    ...
    addSwitchThmem:function(menu){
    	this.taskbar.startMenu.addMenuItem(menu);
    	this.taskbar.add('<center><img src="resources/images/iteLogo.png"/></center>');
    },
    //增加机构切换
    addSwitchOrgan:function(menu){
    	this.taskbar.startMenu.addMenuItem(menu);
    },
    //增加系统切换
    addSwitchSystem:function(menu){
    	this.taskbar.startMenu.addMenuItem("-");
    	this.taskbar.startMenu.addMenuItem(menu);
    },
    //增加功能菜单
    addModuleMenu:function(menus){
    	...
    	for(var i=0;i<menus.length;i++){
    		this.taskbar.startMenu.addMenuItem(menus[i]);
    	}
    	this.taskbar.startMenu.addMenuItem("-");
    },
    setLoginTitle:function(title){
    	this.taskbar.startMenu.title = title;
    },
    ...
    createDesktopMenu: function () {
        ...
        ret.items.push(
                { text: '窗口并列', handler: me.tileWindows, scope: me, minWindows: 1 },
    ...
    createWindow: function(config, cls) {
        var me = this, win, cfg = Ext.applyIf(config || {}, {
                stateful: false,
                isWindow: true,
                constrainHeader: true,//把窗口限制在可见区域
                constrain:true,
                minimizable: true,
                maximizable: true
            });
