...
Ext.define('Ext.ux.form.TimePickerField', {  
    ...
    style : 'padding:4px 0 0 0;margin-bottom:0px;width:200px',  
...
Ext.define('Ext.ux.DateTimePicker', {  
    ...
    onRender : function(container, position) {  
        ...
        //var table = this.el.child('table.x-datepicker-inner');
        //alert(table.dom.outerHTML);
        ...
        me.okBtn = new Ext.button.Button({  
            text: '确定',  
            tooltip: '确定', 
            handler:function(){me.isOkHandler = true;me.okHandler();},
            scope: me,
            renderTo : this.el.child('div div.ux-timefield').parent().next()
        });
    ...
    okHandler : function(){  
        var me = this, btn = me.okBtn, handler = me.handler;  
        if (btn && !btn.disabled) { 
        ...
        return Ext.create('Ext.ux.DateTimePicker', {  
            ...
            listeners : {  
                ...
                select : function(a,c){
                	...
                	//设置选中的样式 --开始 ----
                	var select_A = Ext.get(Ext.DomQuery.selectNode("a[@class='x-datepicker-date']", a.el.dom)); 
                	var trlist = select_A.parent().parent().parent().el.dom.childNodes;
                	var isSetClass = false;
                	for(var i=0;i<trlist.length;i++){
                		var tdlist = trlist[i].childNodes;
                		for(var j=0;j<tdlist.length;j++){
                			var _select_A = tdlist[j].childNodes[0];
                			var vv = _select_A.innerHTML;
                			if(vv.indexOf("<")!=-1){
                				vv= vv.substring(0,vv.indexOf("<"));
                			}
                			if(Number(vv) == Number(c.getDate())){
                				tdlist[j].className = (tdlist[j].className+" x-datepicker-selected ");
                				isSetClass = true;
                				break;
                			}
                		}
