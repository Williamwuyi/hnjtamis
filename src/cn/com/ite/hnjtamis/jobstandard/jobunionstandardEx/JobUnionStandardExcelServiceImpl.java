 ...
 * 岗位标准导出
 * @author 朱健
 * @create time: 2016年3月23日 下午3:42:43
 * @version 1.0
 * 
 * @modified records:
 */
public class JobUnionStandardExcelServiceImpl extends ExportExcelServiceImpl {
	...
	public List setHeaderForExcel(Object form,int index) {
		...
		List<StandardTerms> standardTermslist= (List<StandardTerms>)valueMap.get("standardTermslist");
        ...
        for(int j=0;j<standardTermslist.size();j++){
	        ...
	        if(!oldParentTypeName.equals(standardTermsForm.getParentTypeName())){
	        	int rowspan = 1;
	        	for(int i=j+1;i<standardTermslist.size();i++){
	        		StandardTerms tt = (StandardTerms)standardTermslist.get(i);
	        		if(tt.getParentTypeName().equals(standardTermsForm.getParentTypeName())){
	        			rowspan++;
	        		}else{
	        			break;
	        		}
	        ...
	        oldParentTypeName = standardTermsForm.getParentTypeName();
	        col++;
