...
public class JsonUtils {
		...
		builder.registerTypeAdapter(java.util.Date.class , new JsonDeserializer<Date>(){
					public Date deserialize(JsonElement json, Type typeOfT,  
						...
						if(date.length()==10)
							...
							patter = "yyyy-MM-dd HH";
						else if(date.length()==16)
							patter = "yyyy-MM-dd HH:mm";
						else if(date.length()==19)
							patter = "yyyy-MM-dd HH:mm:ss";
						return DateUtils.convertStrToDate(date, patter);
	...
	/**
	 * 将从前台提交过来的json 字符串参数转换成指定的类型对象。
	 * <strong>此方法通常用来转换普通的 {@code JavaBean}
	 * 对象。</strong>
	 * 
	 * @param <T> 要转换的目标类型。
	 * @param json 给定的 {@code JSON} 字符串。
	 ...
	 * @return 给定的 {@code JSON} 字符串表示的指定的类型对象。
	 */
	public static <T> T fromWebJson(String parameterName, TypeToken<T> clazz) {
		String json = ((String[])ServletContent.getParameter().get(parameterName))[0];
		//去掉空数据
		json = json.replaceAll(",\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\"\\}", "\\}");
		json = json.replaceAll("\\{\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\",", "\\{");
		json = json.replaceAll(",\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\"", "");
		return fromJson(json, clazz, null);
	}
	
	public static <T> T fromWebJson(String parameterName, Class<T> clazz) {
		String json = ((String[])ServletContent.getParameter().get(parameterName))[0];
		//去掉空数据
		while(StringUtils.regex("\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\"",json)){
			json = json.replaceAll(",\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\"\\}", "\\}");
			json = json.replaceAll("\\{\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\",", "\\{");
			json = json.replaceAll(",\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\",", ",");
			json = json.replaceAll("\\{\"[A-Za-z][A-Za-z0-9_]{1,}\":\"\"\\}", "\\{\\}");
		...
		//去掉数组中的空元素
		while(StringUtils.regex("\\{\\}",json)){
			json = json.replaceAll("\\[\\{\\},", "\\[");
			json = json.replaceAll(",\\{\\}\\]", "\\]");
			json = json.replaceAll(",\\{\\},", ",");
		}
		//去年JSON参数
		json = json.replaceAll(",\"json\":\"[A-Za-z][A-Za-z0-9_]{1,}\"", "");
		return fromJson(json, clazz, null);
	}
